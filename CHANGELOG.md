...
- 任务指派支持多人：`task assign <id> <账号或姓名>[,<账号或姓名>...]` 与 `task edit --assigned-to` 现接受多个值（逗号分隔或重复 flag，逐个姓名解析并去重），按**整体替换**语义提交团队——多人落为团队任务（成员工时沿用原值，新增成员 0），单人则转回单人任务并清空团队。真实冒烟确认：多人必须同时提交 `multiple=1` + `assignedTo=首成员` + 配对 `team*[]` 数组，否则服务端拒绝「多人任务不能指派给任务团队外的成员」。
- `task get` 新增「团队成员」字段：多人任务按禅道 `order` 升序列出成员账号（单人任务不显示该行）；JSON 输出新增 `team` 数组（空时省略）。

### 修复

- **不指派则不创建**：`task create` 未提供 `--assigned-to` 时服务端虽回「保存成功」但**实际不落库**，CLI 此前仍打印「已创建任务」。现前置拒绝（退出码 6，提示必须指定指派人），避免误报创建成功。
